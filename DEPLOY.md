...
## :material-lock: 비밀번호 보호 (StatiCrypt)

`/2026-summer/` 이하 모든 페이지는 **비밀번호로 암호화**되어 있습니다.
랜딩 (`/`)은 공개. 사이트 전체에 `noindex` 메타가 들어가 있어 구글 검색에는
뜨지 않습니다.

### 현재 비밀번호

**`hydroai2026`** (테스트용 — 학생들에게 공유 전에 꼭 교체)

### 비밀번호 교체하기

1. GitHub repo 페이지 → **Settings** → 왼쪽 **Secrets and variables** → **Actions**
2. 우측 상단 **"New repository secret"** 버튼
3. **Name**: `STATICRYPT_PASSWORD`
4. **Secret**: 원하는 비밀번호 (예: `hydroai-gsurf-2026!`)
5. **"Add secret"** 클릭
6. **Actions** 탭 → 가장 최근 워크플로우 → 우측 **"Re-run all jobs"**
   (또는 아무 markdown 파일 작은 수정 후 push)

새 비밀번호로 즉시 재배포됩니다. Secret을 안 설정한 상태에선 기본
`hydroai2026`이 쓰입니다 (워크플로우 로그에 경고 표시).

### 학생에게 비밀번호 전달

Slack DM으로 보내거나, 미팅에서 직접 알려주세요. **이메일/Doc에 평문으로
적어두지 마세요** — 평문 비밀번호가 새면 사이트 접근이 풀립니다.

### StatiCrypt 한계 (꼭 알아두기)

- 클라이언트 측 암호화입니다 → **진짜 비밀 데이터는 올리지 마세요**.
  비밀번호가 새면 누구나 복호화 가능합니다.
- 학생 이름·연구 주제·미팅 안건 정도의 "외부 노출 차단" 용도엔 충분합니다.
- 미발표 결과/연구비 정보/개인정보는 여전히 사이트에 올리면 안 됩니다.

### 보호 범위 바꾸기

지금은 `/2026-summer/` 이하만 보호. 사이트 전체 보호로 바꾸려면
`.github/workflows/deploy.yml`에서 staticrypt 명령어의 마지막 인자
`"site/2026-summer/"`를 `"site/"`로 변경.
